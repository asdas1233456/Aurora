"""Local answer-building utilities used by the mock provider."""
...
def _build_local_answer(question: str, retrieved_chunks: list[RetrievedChunk]) -> str:
    ...
    for index, point in enumerate(answer_points[: 2 if is_step_question else 3], start=1):
...
def _looks_like_command(candidate: str) -> bool:
    ...
    if any(
        ...
        return True
    return bool(re.search(r"[|><=/-]", normalized_candidate) and re.search(r"[a-z]", normalized_candidate))
...
def _looks_like_step_question(question: str) -> bool:
    ...
    return any(marker in normalized_question for marker in ("怎么", "如何", "步骤", "排查", "定位", "处理", "查看"))
