...
class AppConfig:
    ...
    tenant_id: str = field(default_factory=lambda: _get_env("TENANT_ID", default="aurora_internal"))
    deployment_mode: str = field(
        default_factory=lambda: _get_env("DEPLOYMENT_MODE", default="single_instance")
    )
    auth_mode: str = field(default_factory=lambda: _get_env("AUTH_MODE", default="development").lower())
    auth_header_user_id: str = field(
        default_factory=lambda: _get_env("AUTH_HEADER_USER_ID", default="x-auth-request-user")
    )
    auth_header_display_name: str = field(
        default_factory=lambda: _get_env("AUTH_HEADER_DISPLAY_NAME", default="x-auth-request-name")
    )
    auth_header_email: str = field(
        default_factory=lambda: _get_env("AUTH_HEADER_EMAIL", default="x-auth-request-email")
    )
    auth_header_role: str = field(
        default_factory=lambda: _get_env("AUTH_HEADER_ROLE", default="x-auth-request-role")
    )
    auth_header_team_id: str = field(
        default_factory=lambda: _get_env("AUTH_HEADER_TEAM_ID", default="x-auth-request-team")
    )
    auth_header_project_ids: str = field(
        default_factory=lambda: _get_env("AUTH_HEADER_PROJECT_IDS", default="x-auth-request-projects")
    )
    auth_active_project_header: str = field(
        default_factory=lambda: _get_env("AUTH_ACTIVE_PROJECT_HEADER", default="x-aurora-project-id")
    )
    auth_dev_user_id: str = field(
        default_factory=lambda: _get_env("AUTH_DEV_USER_ID", default="aurora-admin")
    )
    auth_dev_display_name: str = field(
        default_factory=lambda: _get_env("AUTH_DEV_DISPLAY_NAME", default="Aurora Admin")
    )
    auth_dev_email: str = field(
        default_factory=lambda: _get_env("AUTH_DEV_EMAIL", default="aurora-admin@example.internal")
    )
    auth_dev_role: str = field(default_factory=lambda: _get_env("AUTH_DEV_ROLE", default="admin"))
    auth_dev_team_id: str = field(
        default_factory=lambda: _get_env("AUTH_DEV_TEAM_ID", default="team-platform")
    )
    auth_dev_project_ids: str = field(
        default_factory=lambda: _get_env("AUTH_DEV_PROJECT_IDS", default="aurora")
    )
    upload_max_file_bytes: int = field(
        default_factory=lambda: int(_get_env("UPLOAD_MAX_FILE_BYTES", default=str(10 * 1024 * 1024)))
    )
    upload_max_batch_files: int = field(
        default_factory=lambda: int(_get_env("UPLOAD_MAX_BATCH_FILES", default="10"))
    )
    upload_quarantine_enabled: bool = field(
        default_factory=lambda: _get_env_bool("UPLOAD_QUARANTINE_ENABLED", default=True)
    )
    upload_quarantine_dir: Path = field(
        default_factory=lambda: BASE_DIR / _get_env("UPLOAD_QUARANTINE_DIR", default="quarantine")
    )
    app_name: str = "Aurora"
    app_title: str = "Aurora - 软件测试知识工作台"
    app_version: str = "v0.9.0"
    ...
    def auth_enabled(self) -> bool:
        return self.auth_mode in {"development", "trusted_header"}

    def ensure_directories(self) -> None:
        self.data_dir.mkdir(parents=True, exist_ok=True)
        self.db_dir.mkdir(parents=True, exist_ok=True)
        self.logs_dir.mkdir(parents=True, exist_ok=True)
        self.upload_quarantine_dir.mkdir(parents=True, exist_ok=True)
...
def get_config() -> AppConfig:
    ...
    try:
        from app.services.settings_service import apply_runtime_settings_overrides

        config = apply_runtime_settings_overrides(config)
    except Exception:
        pass
    return config
