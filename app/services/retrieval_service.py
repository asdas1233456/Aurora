...
def retrieve_chunks(
    ...
    if config.embedding_api_ready and get_vector_collection_count(config) > 0:
        index = load_index(config)
        retriever = index.as_retriever(similarity_top_k=min(max(target_top_k * 3, target_top_k), 20))
        nodes = retriever.retrieve(retrieval_query)
        normalized_chunks = [_normalize_node(node) for node in nodes]
    else:
        normalized_chunks = _retrieve_local_chunks(
            question=question,
    ...
    *,
    ...
    for item in load_local_index_chunks(config):
        for candidate in _iter_local_candidates(item):
            ...
            candidate_key = (source_path, text)
